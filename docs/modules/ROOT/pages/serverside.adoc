...
This code is generated automatically from webidl files. These classes are separated by 3 main groups: eventTarget, event, callback. +
...
browser) and execute some callbacks when a user activates his presentation on the browser. This type of callback is done by using an on-event handler attribute(like onclick, on keypress, onfocus,etc) 
